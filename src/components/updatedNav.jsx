      ...
      <div className='m-auto lg:m-0 flex justify-between items-center p-2 text-black'>
        ...
        >
          {links.map((link) => (
            <Link
              key={link.href}
              className='p-4 no-underline hover:underline hover:text-slate-700 dark:hover:text-white rounded-md text-xl font-semibold'
              href={link.href}
            >
              {link.label}
            </Link>
        ...
        >
          <ul>
            {/* <li
              ...
              onClick={handleNav}
            >
              {mobileLinks.map((mobileLinks) => (
                <Link
                  key={mobileLinks.href}
